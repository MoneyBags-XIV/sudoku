    ...
    def solve_by_box(self):
        ...
        while True:
            ...
            for box_num in range(9):
                ...
                for i, possible_coord in enumerate(number_can_coords):
                    qwer = [possible_coord]
                    uiop = [i+1]
                    for j in range(9):
                        if j == i:
                            continue
                        if tuple(number_can_coords[j]) == tuple(possible_coord):
                            qwer.append(number_can_coords[j])
                            uiop.append(j+1)
                    if len(qwer) == len(possible_coord):
                        for coord in possible_coord:
                            for x in range(1, 10):
                                if not x in uiop:
                                    try:
                                        self.candidates[coord[0]][coord[1]].remove(x)
                                    except:
                                        pass
        ...
        while True:
            ...
            for i, row in enumerate(self.grid):
                ...
                for k in range(9):
                    ...
                    ghghgh = [[i, x] for x in possible_indexes]
